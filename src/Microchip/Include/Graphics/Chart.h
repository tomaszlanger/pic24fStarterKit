...
Sample Axis - This is the horizontal range of a chart for normal bar charts and 
			  ...
			  sample segments, or other non-numeric data.
...
// To be added
//	XCHAR 		   *pCatLabel;		// Pointer to the array of category labels used for the samples axis.
//									// If this is NULL labels are numeric (i.e. 1, 2, 3, ...etc).
...
*********************************************************************/
typedef struct {
	OBJ_HEADER      hdr;			// Structure for the dimension and style scheme used
									// by the chart object.
...
*		state = CH_BAR|CH_DRAW|CH_COLUMN;
*
*		Contents.pTitle  = ChTitle;
*		Contents.pSmplLabel = SampleName;
*		Contents.pValLabel = ValueName;
*		Contents.pVarNameLst = VarName;
*		Contents.seriesCount = 2;			// only V1 and V2 samples.
*
*		pChart = ChCreate(0x01,10,10,100,100,state, &Contents, NULL, pScheme);
*		if (pChart == NULL)		
*		   return 0;
*		else
*			return 1;
*
*		// add the data series and their data samples. 
*		// create the link list of variable structures.
*		ChAddDataSeries(pChart, 2, V1Data, SeriesName[0]); 
*		ChAddDataSeries(pChart, 2, V2Data, SeriesName[1]);
*
*		// set the range of the values
*		ChSetValRange(pChart, 0, 50);
...
// NOT YET IMPLEMENTED
//#define ChSetCategoryLabel(pCh, pLabel) (pCh->prm.pCatLabel = pLabel)
//#define ChGetCategoryLabel(pCh) 		(pCh->prm.pCatLabel)
...
*
* Overview: This function sets the minimum and maximum value of the
*			bar charts. The criteria is that min <= max.
*
* PreCondition: none
*
* Input: pCh - Pointer to the chart object.
...
*
* Overview: This macro gets the gets the value range for bar charts. 
...
*
* Overview: This function sets the minimum and maximum value of the
*			percentage range for bar charts. The criteria is that min <= max.
...
*			The value returned is calculated from max - min.
*
* PreCondition: none
*
* Input: pCh - Pointer to the chart object.
...
*			drawing the chart. Together with the data series's SHOW_DATA
