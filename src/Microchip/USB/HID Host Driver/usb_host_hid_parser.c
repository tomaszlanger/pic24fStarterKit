...
USB_HID_RPT_DESC_ERROR _USBHostHID_Parse_Report(BYTE* hidReportDescriptor , WORD lengthOfDescriptor , WORD pollRate, BYTE interfaceNum)
{
   ...
   if((hidReportDescriptor == NULL) ||(lengthOfDescriptor == 0))
    ...
    deviceRptInfo.reportPollingRate = 
...
//!@#$%    if (deviceRptInfo.globals.logicalMinimum > deviceRptInfo.globals.logicalMaximum)return(HID_ERR_BadLogical); The barcode scanner has this issue.  We'll ignore it.
