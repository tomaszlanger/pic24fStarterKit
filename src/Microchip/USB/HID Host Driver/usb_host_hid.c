...
// For MASTERs Demo only!

#undef USB_ENABLE_TRANSFER_EVENT
...
BYTE USBHostHIDResetDevice( BYTE deviceAddress )
{
    ...
    if (i == USB_MAX_HID_DEVICES)
    {
       ...
       if (((deviceInfo[i].state & STATE_MASK) != STATE_DETACHED) &&
        ...
        #ifndef USB_ENABLE_TRANSFER_EVENT
            deviceInfo[i].returnState = STATE_HOLDING;
        #else
                              ...
                              if(deviceInfo[i].HIDparserError)
                                {
                                  /* Report Descriptor is flawed , flag error and free memory , 
                                    ...
                                    UART2PrintString("\n\rError:  "); 
                                    UART2PutHex( deviceInfo[i].HIDparserError >>  8);
                                    UART2PutHex( deviceInfo[i].HIDparserError );
                                    //UART2PutHex( deviceInfo[i].errorCode >>  8);
                                    //UART2PutHex( deviceInfo[i].errorCode );
...
//                                    if(pCurrInterfaceDetails->next != NULL)
//                                    {
...
//                                    i += descriptor[i];
                                ...
                                {
                                    /* no corresponding endpoint descriptor */
                                }
